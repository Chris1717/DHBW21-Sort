class SelectionSort {
    ...
    public static void sort(int[] data) {
        ...
        for(int target = 0; target < data.length-1; target++) {
            prettyPrint(data);
            int smallest = target;
            for(int currentIdx = target+1; currentIdx < data.length; currentIdx++) {
                if (data[currentIdx] < data[smallest]) {
                    smallest = currentIdx;
                }
            }
            swap(data, target, smallest);
        }
        prettyPrint(data);
    }
